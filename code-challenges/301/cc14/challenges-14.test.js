...
let biggerThanLuke = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++){
      ...
      lukeWeight = parseInt(arr[i].mass);
      }
  }

  arr.forEach((char) =>{
      if (char.mass > lukeWeight){
      heavier.push(char.name);
      }
  });
  return heavier.join(' - ');
}
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i = 0; i < board[0].length; i++) {
    ...
    // horizontals
    const dataOne = [];
    for (let j = 0; j < board[0].length; j++){ dataOne.push(board[i][j]) }
    if (helpCheck(dataOne) === true){ return true }
    
    // verticals
    const dataTwo = [];
    for (let j = 0; j < board[0].length; j++){ dataTwo.push(board[j][i]) }
    if (helpCheck(dataTwo) === true){ return true }
    
    // diagonals
    dataThree.push(board[i][i]);
    dataFour.push(board[2-i][i]);
  }
  if (helpCheck(dataThree) === true){ return true }
  if (helpCheck(dataFour) === true){ return true }
  return false;
}

// x1 y1, x2, y2, x3, y3
const helpCheck = function(data){
