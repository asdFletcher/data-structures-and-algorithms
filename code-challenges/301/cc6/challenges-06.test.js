...
const hasChildrenEntries = (arr, character) => {
  ...
  arr.forEach( (val) => {
    if (val.name === character){
      if (Object.values(val.children)[0]) {
        hasChildren = true;
      }
    }
  });
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach( (val) => {
    let count = 1;
    count = count + val.children.length;
    if (val.spouse) { count = count + 1 };
    if (deceasedSpouses.includes(val.spouse)){ count = count - 1 };
    survivors.push({
      house: val.house,
      members: count}
    );
