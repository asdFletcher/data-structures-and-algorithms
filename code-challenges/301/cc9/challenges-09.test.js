...
const returnNames = (arr) => {
  return arr.reduce( (total, currentCharObj) => {
    total.push(currentCharObj.name)
    return total;
  }, []);
};
...
const reversedString = (string) => {
  ...
  let arrReversed = arr.reduce( (total, current) => {
...
const countNumberOfChildren = (characters) => {
...
const calculateAverage = (arr) => {
  let data = arr.reduce( (accumulator, current) => {
    accumulator.count ++;
...
const extractStat = (statName, arr) => {
  return arr.reduce((total, current, idx, arr)=>{
...
const extractChildren = (arr) => {
  let aNames = arr.filter( (char) => char.name.match(/a/g) );
  
  return aNames.reduce((total, character)=>{
    if (character.children !== undefined){
      character.children.forEach( (child) => total.push(child));
    };
    return total;
  },[]);
