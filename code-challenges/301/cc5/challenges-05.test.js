...
const listFoods = (recipe) => {
  ...
  ingrArray.forEach( (ingredientFull) => {
    let ingr = ingredientFull;
    
    // identify location of first space
    let firstSpaceIndex = ingr.indexOf(' ') + 1;
    
    // trim that portion off
    ingr = ingr.slice(firstSpaceIndex);
    
    // identify location of second space
    let secondSpaceIndex = ingr.indexOf(' ') + 1;
    
    // trim that portion off
    ingr = ingr.slice(secondSpaceIndex);
...
const splitFoods = (recipe) => {
  ...
  ingrArray.forEach( (ingredientFull) => {
    ...
    // split on spaces
    let ingrSplit = ingredientFull.split(' '); 

    // remove elements before 2nd space 
    let ingrNameSplit = ingrSplit.slice(2);

    // join together remaining with space
    result.push(ingrNameSplit.join(' '));
...
const stepActions = (recipe) => {
  ...
  stepsArray.forEach((step) => {
    ...
    // add first element from split on spaces
...
const removeEvenValues = (arr) => {
  ...
  // put the stored numbers into the array
  result.forEach( (num) => arr.push(num));
};
...
const extractVowels = (str) => {
  ...
  // create, sort and join vowels array
  let vowels = str.match(regexVowels);
  vowels.sort();
  vowels = vowels.join('');

  // create and add new string
  const stringWithNoVowels = str.replace(regexVowels, '')
  
  
  return [stringWithNoVowels, vowels];
