...
const citiesAtoJ = (arr) => {
  ...
  var regex = /^[A-Ja-j][A-Za-z ]+/gm;

  // join the array with LINE BREAK deliniation so that ^ works on each city separately
  // then create array with regex 
  return arr.join('\n').match(regex);
...
const matchMonth = (input) => {
  ...
  var regex = /^[OCToct]ct(ober){0,1}\b/g;
