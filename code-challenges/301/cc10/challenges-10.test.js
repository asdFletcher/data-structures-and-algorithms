...
const count = (target, input) => {
    // outside reduce
    let output = input.reduce((total, currentArray) => {
      
      // inside reduce
      total += currentArray.reduce( (count, num) => {
        if (num === target) {count++};
        return count;
      }, 0);
...
const divisibleByFiveTwoToThePower = (outerArray) => {
  
  return outerArray.map( (innerArray) => {
    return innerArray.reduce( (result, num) => {
...
let findMaleAndFemale = (data) => {
  return data.reduce((string, object, index, array) => {
...
let findShortest = (data) => {
  ...
  return data.reduce( (shortestCharacter, currentCharacter) => {
    
    if (parseInt(currentCharacter.height) < parseInt(shortestCharacter.height) ){
       shortestCharacter = currentCharacter;
    }
